...
impl fmt::Debug for BtcError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let error_as_str = match self {
            ...
            Self::NumMustBeLessThanPrimeOrder => {
                "The `self.num` field must be less than the `self.prime` field"
            }
            Self::PrimeOrderMustBeEqual => {
                "Cannot add two numbers in different Fields. Prime
